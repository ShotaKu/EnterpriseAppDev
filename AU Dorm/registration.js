                    ...
                    if (newOwner != null) {
                        ...
                        if (empty(dName)) {
                            ...
                            if(uploadFinish != null){
                                return uploadFinish.then(function (dThumbURL) {
                                ...
                                }).then(function(newDorm){
                                    if(newDorm != null){
                                        createNewRoom(guid(),newDorm.rPoint,newDorm.price,newDorm.area,newDorm.rThumb,newDorm.rLayout,newDorm.rooms)
                                        .then(function(newRoom){
                                            firebase.auth().createUserWithEmailAndPassword(email, password).then(function(s){
                                                var StaffRef = database.ref("Staff");
                                                var d = new Date();
                                                var ownerID = pushAt(database,"Staff",newOwner);
                                                var newData = {
                                                    owner:ownerID
                                                    ,information:newDorm.newDorm
                                                    ,lastEdit:d.getTime()
                                                    ,newRoom
                                                }
